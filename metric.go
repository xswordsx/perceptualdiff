...
type PerceptualDiffParameters struct {
	...
	threshold_pixels uint

	// How much color to use in the metric.
	// 0.0 is the same as luminance_only_ = true,
	// 1.0 means full strength.
	color_factor float64
}

var global_white struct {
	x, y, z float64
}
var args PerceptualDiffParameters
...
func adaptation(num_one_degree_pixels float64) uint {
	num_pixels := 1.0
	adaptation_level := uint(0)
